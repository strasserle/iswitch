...
Data_tsv = "data\\tcga_Kallisto_tpm.gz.part"
...
abundance = pd.read_csv(Data_tsv, sep="\t", compression = "gzip", usecols=lambda x: x in set(["sample"] + needed_samples), nrows = 1000) 
...
def example_plot_switches(gene_id, switch_pair_df): 
...
def example_plot_switches_reordered(gene_id, switch_pair_df): 
    pairs = switch_pair_df[switch_pair_df.Symbol == gene_id]
...
biogrid_human_path = ZipFile(biogrid_path).extract(member = "BIOGRID-ORGANISM-Homo_sapiens-4.2.193.tab3.txt", path ="data")

# read file
biogrid_human = pd.read_csv(biogrid_human_path, sep = "\t", usecols=["#BioGRID Interaction ID", "Official Symbol Interactor A", "Official Symbol Interactor B"])
...
def draw_protein_graph(complex_name, switch_df): 
