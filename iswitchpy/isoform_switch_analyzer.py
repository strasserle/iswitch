...
def detect_iswitches(method="isa",
    ...
    *args: 
        parameters of the according isoform switch detection methods
    """
    ...
    if method == "isa":
        ...
        return detect_iswitches_spada(pheno, data, disease, quiet)




def detect_iswitches_spada(pheno, data, disease, signif_threshold=0.3, quiet=False):
    ...
    abundance = pd.read_csv(data, sep="\t", compression="gzip", usecols=lambda x: x in set(["sample"] + needed_samples), nrows=1000, index_col=0)


    # create input files for spada
    abundance.loc[:, needed_cases].to_csv(disease.replace(" ", "_") + 'Case.tsv', sep='\t', index=False)
    abundance.loc[:, needed_controls].to_csv(disease.replace(" ", "_") + 'Control.tsv', sep='\t', index=False)
    
    if not quiet:
        print("Starting SPADA run ...")

    os.system('python spada switches --expression-control kidney_clear_cell_carcinomaControl.tsv --expression-case kidney_clear_cell_carcinomaCase.tsv')

    # sys.path.extend(['C:\\Users\\stras\\anaconda3\\envs\\iswitch\\scripts'])
    # sys.path.extend(["C:\\Users\\stras\\anaconda3\\envs\\iswitch\\Scripts\\"])
    # sys.path.extend([" C:\\Users\\stras\\appdata\local\\programs\\python\\python37-32\\scripts\\spada"])
    # os.system('python C:\\Users\\stras\\anaconda3\\envs\\iswitch\\Scripts\\spada switches --expression-control C:\\Users\\stras\\Documents\\8.Semester\\FortgeschrittenenPraktikum\\iswitchpy\\iswitchpy\\' + disease.replace(" ", "_") + 'Control.tsv --expression-case ' + disease.replace(" ", "_") + 'Case.tsv')
    # os.system('spada switches --expression-control ' + disease.replace(" ", "_") + 'Control.tsv --expression-case ' + disease.replace(" ", "_") + 'Case.tsv')

    # os.system('python plotting.py ' + disease +'Case.tsv switches_spada.tsv')
    # os.system('python outputEditor.py significantSwitches_spada.tsv')
    # os.system('python enrichment.py')



def detect_iswitches_isa(pheno, data, disease, anno, dIFcutoff=0.1, alpha=0.05, foldChangePseudoCount=0.01, quiet=False):
